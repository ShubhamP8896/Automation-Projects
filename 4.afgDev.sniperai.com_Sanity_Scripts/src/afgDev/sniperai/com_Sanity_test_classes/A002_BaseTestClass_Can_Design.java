...
import afg_dev.sniperai.com_utility_Classes.AfgDev_Take_Screenshot_UtilityClass;
...
public class A002_BaseTestClass_Can_Design 
{
    ...
    private void navigateToUrl() {
        driver.get("https://afgdev.sniperai.com/login");
...
//          Excel_Data_UtilityClass.writeResult("Test case failed - Incorrect credentials. Alert message: " + alertMessage);
            ...
            AfgDev_Excel_Data_UtilityClass.writeResult("No web element found on the page.");
        } catch (WebDriverException e) {
            System.out.println("WebDriverException occurred: " + e.getMessage());
            AfgDev_Excel_Data_UtilityClass.writeResult("WebDriverException occurred: " + e.getMessage());
        } catch (Exception e) {
            System.out.println("An exception occurred: " + e.getMessage());
            AfgDev_Excel_Data_UtilityClass.writeResult("An exception occurred: " + e.getMessage());
        }
    }
    ...
    public void teardown(ITestResult result) 
    {
        try 
        {
            ...
            {
                AfgDev_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName() + "_Failure_Screenshot");
                System.out.println("Failure Screenshot taken Successfully");
            } 
            else 
            {
                AfgDev_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName());
