...
import damacgrouppoc.sniperai.com_utility_Classes.Ddamacgrouppoc_Take_Screenshot_UtilityClass;
import damacgrouppoc_CanMod_POM_classes.Can_DashboardPage;
import damacgrouppoc_Sanity_POM_Classes.LoginPage;
...
public class A002_BaseTestClass_Can_Design 
{
    ...
    private void navigateToUrl() {
        driver.get("https://damacgrouppoc.sniperai.com/login");
    ...
    private void performLogin() throws IOException {
        ...
        LP.fetchCandidateCredentials(26);
        LP.CMSendUsername();
...
//          Excel_Data_UtilityClass.writeResult("Test case failed - Incorrect credentials. Alert message: " + alertMessage);
            ...
            Damacgrouppoc_Excel_Data_UtilityClass.writeResult("No web element found on the page.");
        } catch (WebDriverException e) {
            System.out.println("WebDriverException occurred: " + e.getMessage());
            Damacgrouppoc_Excel_Data_UtilityClass.writeResult("WebDriverException occurred: " + e.getMessage());
        } catch (Exception e) {
            System.out.println("An exception occurred: " + e.getMessage());
            Damacgrouppoc_Excel_Data_UtilityClass.writeResult("An exception occurred: " + e.getMessage());
        }
    }
    ...
    public void teardown(ITestResult result) 
    {
        try 
        {
            ...
            {
                Ddamacgrouppoc_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName() + "_Failure_Screenshot");
                System.out.println("Failure Screenshot taken Successfully");
            } 
            else 
            {
                Ddamacgrouppoc_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName());
