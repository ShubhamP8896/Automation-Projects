...
import bcforwardpoc.sniperai.com_utility_Classes.Bcforwardpoc_Take_Screenshot_UtilityClass;
import bcforwardpoc_RecMod_POM_classes.Rec_JobListPage_POM_Class;
import bcforwardpoc_Sanity_POM_Classes.LoginPage;
...
public class A003_BaseTestClass_Rec_Design 
{
    ...
    {
        driver.get("https://bcforwardpoc.sniperai.com/");
    ...
    private void performLogin() throws IOException 
    {
        ...
        LP.fetchRecruiterCredentials(47);
...
//          Excel_Data_UtilityClass.writeResult("Test case failed - Incorrect credentials. Alert message: " + alertMessage);
            ...
            Bcforwardpoc_Excel_Data_UtilityClass.writeResult("No web element found on the page.");
        } catch (WebDriverException e) {
            System.out.println("WebDriverException occurred: " + e.getMessage());
            Bcforwardpoc_Excel_Data_UtilityClass.writeResult("WebDriverException occurred: " + e.getMessage());
        } catch (Exception e) {
            System.out.println("An exception occurred: " + e.getMessage());
            Bcforwardpoc_Excel_Data_UtilityClass.writeResult("An exception occurred: " + e.getMessage());
        }
    }
    ...
    public void teardown(ITestResult result) 
    {
        try 
        {
            ...
            {
                Bcforwardpoc_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName() + "_Failure_Screenshot");
                System.out.println("Failure Screenshot taken Successfully");
            } 
            else 
            {
                Bcforwardpoc_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName());
