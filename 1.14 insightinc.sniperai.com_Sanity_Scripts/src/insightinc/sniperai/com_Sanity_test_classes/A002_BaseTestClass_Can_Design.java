...
import insightinc.sniperai.com_utility_Classes.Insightinc_Take_Screenshot_UtilityClass;
import insightinc_CanMod_POM_classes.Can_DashboardPage;
import insightinc_Sanity_POM_Classes.LoginPage;
...
public class A002_BaseTestClass_Can_Design 
{
    ...
    private void navigateToUrl() {
        driver.get("https://insightinc.sniperai.com/");
    ...
    private void performLogin() throws IOException {
        ...
        LP.fetchCandidateCredentials(30);
        LP.CMSendUsername();
...
//          Excel_Data_UtilityClass.writeResult("Test case failed - Incorrect credentials. Alert message: " + alertMessage);
            ...
            Iinsightinc_Excel_Data_UtilityClass.writeResult("No web element found on the page.");
        } catch (WebDriverException e) {
            System.out.println("WebDriverException occurred: " + e.getMessage());
            Iinsightinc_Excel_Data_UtilityClass.writeResult("WebDriverException occurred: " + e.getMessage());
        } catch (Exception e) {
            System.out.println("An exception occurred: " + e.getMessage());
            Iinsightinc_Excel_Data_UtilityClass.writeResult("An exception occurred: " + e.getMessage());
        }
    }
    ...
    public void teardown(ITestResult result) 
    {
        try 
        {
            ...
            {
                Insightinc_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName() + "_Failure_Screenshot");
                System.out.println("Failure Screenshot taken Successfully");
            } 
            else 
            {
                Insightinc_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName());
