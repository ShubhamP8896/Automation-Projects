...
import mag_preprod.sniperai.com_utility_Classes.MagPreprod_Take_Screenshot_UtilityClass;
...
public class A003_BaseTestClass_Rec_Design 
{
    ...
    {
        driver.get("https://malaysiaairlinespreprod.sniperai.com/");
...
//          Excel_Data_UtilityClass.writeResult("Test case failed - Incorrect credentials. Alert message: " + alertMessage);
            ...
            MagPreprod_Excel_Data_UtilityClass.writeResult("No web element found on the page.");
        } catch (WebDriverException e) {
            System.out.println("WebDriverException occurred: " + e.getMessage());
            MagPreprod_Excel_Data_UtilityClass.writeResult("WebDriverException occurred: " + e.getMessage());
        } catch (Exception e) {
            System.out.println("An exception occurred: " + e.getMessage());
            MagPreprod_Excel_Data_UtilityClass.writeResult("An exception occurred: " + e.getMessage());
        }
    }
    ...
    public void teardown(ITestResult result) 
    {
        try 
        {
            ...
            {
                MagPreprod_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName() + "_Failure_Screenshot");
                System.out.println("Failure Screenshot taken Successfully");
            } 
            else 
            {
                MagPreprod_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName());
