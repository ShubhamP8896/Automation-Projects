...
import recruitmentsmart.sniperai.com_utility_Classes.Recruitmentsmart_Take_Screenshot_UtilityClass;
import recruitmentsmart_CanMod_POM_classes.Can_DashboardPage;
import recruitmentsmart_Sanity_POM_Classes.LoginPage;
...
public class A002_BaseTestClass_Can_Design 
{
    ...
    private void navigateToUrl() {
        driver.get("https://recruitmentsmart.sniperai.com/");
    ...
    private void performLogin() throws IOException {
        ...
        LP.fetchCandidateCredentials(40);
        LP.CMSendUsername();
...
//          Excel_Data_UtilityClass.writeResult("Test case failed - Incorrect credentials. Alert message: " + alertMessage);
            ...
            Recruitmentsmart_Excel_Data_UtilityClass.writeResult("No web element found on the page.");
        } catch (WebDriverException e) {
            System.out.println("WebDriverException occurred: " + e.getMessage());
            Recruitmentsmart_Excel_Data_UtilityClass.writeResult("WebDriverException occurred: " + e.getMessage());
        } catch (Exception e) {
            System.out.println("An exception occurred: " + e.getMessage());
            Recruitmentsmart_Excel_Data_UtilityClass.writeResult("An exception occurred: " + e.getMessage());
        }
    }
    ...
    public void teardown(ITestResult result) 
    {
        try 
        {
            ...
            {
                Recruitmentsmart_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName() + "_Failure_Screenshot");
                System.out.println("Failure Screenshot taken Successfully");
            } 
            else 
            {
                Recruitmentsmart_Take_Screenshot_UtilityClass.takeScreenshot(driver, result.getName());
