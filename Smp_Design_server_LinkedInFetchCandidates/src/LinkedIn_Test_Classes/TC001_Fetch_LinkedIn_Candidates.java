...
//		JobList page POM class object to perform actions on job list page
		...
		// ✅ Notification wait with timeout handling
		WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(75));
		String actualResult;

		try {
			WebElement notification = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[@id='notificationText']")));

			String notificationText = notification.getText();
			System.out.println("Notification received: " + notificationText);

			if (notificationText.contains(
					"Great news! Sniper AI has successfully fetched LinkedIn candidates for Job Req Id: LinkedIn-1.\r\n"
